...
import { DeliveryDriverResponse } from '../../actions/delivery-drivers/delivery-drivers.interface';
...
export default function CreateDeliveryOrderPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [menu, driversData, clientsData] = await Promise.all([
          getMenu(),
          getDeliveryDrivers({ active: true }),
          getClients({ page: 0, size: 100, sort: ['name,asc'], active: true }),
        ]);
  ...
  const total =
    productsTotal + (Number.isNaN(parsedDeliveryFee) ? 0 : parsedDeliveryFee);
  ...
  function handleSubmit() {
    const deliveryFeeNumber = Number(deliveryFee);
    ...
    if (
      ...
      !addressLine.trim() ||
      !phoneNumber.trim() ||
      Number.isNaN(deliveryFeeNumber) ||
      deliveryFeeNumber < 0
    ) {
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-black text-gray-900 uppercase tracking-tighter leading-tight">
              Nueva orden · <span className="text-purple-600">Delivery</span>
            </h1>
            <p className="text-[10px] text-gray-400 uppercase tracking-widest">
              Dirección, conductor y productos
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="bg-white border-b border-gray-100 px-4 py-3 space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 transition-all"
              />
            </div>
              ...
              <div className="grid grid-cols-2 xl:grid-cols-3 gap-3">
                {filteredProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      {qty > 0 && (
                        <span className="absolute top-2 right-2 z-10 w-5 h-5 rounded-full bg-purple-600 text-white text-[10px] font-black flex items-center justify-center shadow">
                          {qty}
                        </span>
                      )}
        ...
        <aside className="w-[430px] bg-white flex flex-col flex-shrink-0">
          <div className="flex-1 overflow-y-auto">
            <section className="px-4 py-3 border-b border-gray-100 space-y-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-black/80 flex items-center gap-1.5">
                <Truck size={12} /> Datos de delivery
              </p>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-black/80 block mb-1">
                    Conductor
                  </label>
                  <select
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-2 text-xs bg-gray-50 border text-black/80 border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all"
                  />
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-2 text-xs text-black/80 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all"
                    />
                  </div>
                </div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 text-xs bg-gray-50 border border-gray-200 text-black/80 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all resize-none placeholder:text-gray-300"
              />
            </section>

            <section className="px-4 py-3 border-b border-gray-100">
              <p className="text-[10px] font-black uppercase tracking-widest text-black/80 flex items-center gap-1.5 mb-2">
                    ...
                    >
                      <div className="flex items-center gap-2 p-2">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() => updateQuantity(item.product.id, 1)}
                            className="w-5 h-5 rounded-md bg-gray-200 hover:bg-purple-100 hover:text-purple-600 flex items-center justify-center transition-colors"
                          >
                            <Plus size={10} />
                          </button>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-black text-purple-700">
                            S/{' '}
                            {(item.product.unitPrice * item.quantity).toFixed(
                              2,
                            )}
                          </span>
                          <button
                      ...
                      {itemDetailsOpen === item.product.id && (
                        <div className="px-2 pb-2">
                          <input
                            ...
                            className="w-full px-2 py-1 text-xs bg-white border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-purple-400 transition-all placeholder:text-gray-300"
                          />
                        </div>
                      )}
                    </div>
          ...
          <div className="border-t border-gray-100 p-4 space-y-3">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 block mb-1 flex items-center gap-1">
                <User size={11} /> Notas de la orden
              </label>
              <textarea
                rows={2}
                ...
                className="w-full px-3 py-2 text-xs bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all resize-none placeholder:text-gray-300"
              />
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-3 space-y-1">
              ...
              <div className="flex justify-between text-sm font-black text-gray-900 border-t border-gray-200 pt-1 mt-1">
                <span>Total</span>
                <span className="text-purple-700">S/ {total.toFixed(2)}</span>
              </div>
            </div>
            ...
            {(!addressLine.trim() || !phoneNumber.trim()) && (
              <p className="text-[10px] text-amber-600 font-bold text-center">
            ...
            >
              {submitting ? 'Enviando…' : 'Confirmar delivery'}
            </button>
          </div>
        </aside>
      </div>
