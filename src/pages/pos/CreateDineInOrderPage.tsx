...
export default function CreateDineInOrderPage() {
  ...
  const [itemDetailsOpen, setItemDetailsOpen] = useState<number | null>(null); // product id
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [menu, tablesData] = await Promise.all([
          getMenu(),
          getTables({ active: true }),
        ]);
        setProducts(menu);
  ...
  function handleSubmit() {
    if (!selectedTable || cart.length === 0 || !wsClient) return;
    ...
    const request: CreateDineInOrderRequest = {
  ...
  const canSubmit = !!selectedTable && cart.length > 0 && !submitting;
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-red-800 flex items-center justify-center animate-pulse">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-black text-gray-900 uppercase tracking-tighter leading-tight">
              Nueva orden · <span className="text-emerald-600">Salón</span>
            </h1>
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="bg-white border-b border-gray-100 px-4 py-3 space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition-all"
              />
            </div>
                ...
                {filteredProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs font-black text-gray-800 leading-tight line-clamp-2">
                        {product.name}
                      </p>
        ...
        {/* ════════════════════ RIGHT: Table + Cart ════════════════════ */}
        <div className="w-[340px] flex-shrink-0 flex flex-col bg-white overflow-hidden">
          {/* ── Table picker ── */}
          <div className="border-b border-gray-100 p-4 space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-7 pr-3 py-1.5 text-xs bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 transition-all"
              />
            </div>
            <div className="grid grid-cols-3 gap-1.5 max-h-36 overflow-y-auto">
              {filteredTables.map((table) => {
                ...
                const disabled = table.status === TableStatus.OUT_OF_SERVICE;
                return (
                  <button
                    key={table.id}
                    disabled={disabled}
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="px-4 pt-4 pb-1 flex items-center justify-between">
              ...
              {cart.length > 0 && (
                <button
                  onClick={() => setCart([])}
                  className="text-[10px] text-red-400 hover:text-red-600 font-bold transition-colors flex items-center gap-0.5"
                >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 p-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() => updateQuantity(item.product.id, 1)}
                          className="w-5 h-5 rounded-md bg-gray-200 hover:bg-emerald-100 hover:text-emerald-600 flex items-center justify-center transition-colors"
                        >
                          <Plus size={10} />
                        </button>
                      </div>
                    ...
                    {/* Expandable detail note */}
                    {itemDetailsOpen === item.product.id && (
                      <div className="px-2 pb-2">
                        <input
                          type="text"
                          placeholder="Notas del ítem (sin cebolla, etc.)"
                          value={item.details}
                          onChange={(e) =>
                            updateItemDetails(item.product.id, e.target.value)
                          }
                          className="w-full px-2 py-1 text-xs bg-white border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-emerald-400 transition-all placeholder:text-gray-300"
                        />
                      </div>
                    )}
                  </div>
          ...
          <div className="border-t border-gray-100 p-4 space-y-3">
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 text-xs bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 transition-all resize-none placeholder:text-gray-300"
              />
            </div>
