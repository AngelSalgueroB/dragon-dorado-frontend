      ...
      <header className="flex justify-between items-center mb-8 border-b border-gray-200 pb-4">
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-2 bg-white border border-gray-300 hover:bg-gray-100 rounded-lg transition-colors text-gray-700 shadow-sm"
          >
            <Home size={20} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-tighter">
              Salón Principal
            </h1>
      ...
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-24 max-w-6xl mx-auto">
        {tablesData.map((table) => (
          <div
            ...
            className={`aspect-square rounded-xl border-2 flex flex-col items-center justify-center cursor-pointer transition-all hover:scale-105 shadow-sm bg-white ${statusStyles[table.status]}`}
          >
      ...
      <footer className="fixed bottom-6 left-6 right-6 bg-white border border-gray-200 rounded-xl p-4 flex justify-between items-center shadow-lg z-20">
        <div className="flex items-center gap-3 text-gray-600">
          ...
          <p className="text-sm italic font-medium">
            Toque una mesa para iniciar pedido
          </p>
        </div>
        <button
          onClick={() => navigate('/cocina')}
          className="bg-chifa-red hover:bg-red-800 text-white px-6 py-2 rounded-lg font-bold text-xs uppercase transition-all shadow-md"
        >
          Ver Cocina (KDS)
        </button>
      </footer>
